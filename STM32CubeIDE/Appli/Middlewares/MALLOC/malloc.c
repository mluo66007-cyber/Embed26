...
/* 内存池定义 */
static uint8_t mem1base[MEM1_MAX_SIZE] __attribute__((aligned(64)));                                        /* AXISRAM1~2内存池 */
static uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((aligned(64))) __attribute__((section(".EXTRAM")));    /* XSPI1 HyperRAM内存池,LTDC使用前1280 * 800 * 2=0x1F4000字节空间 */

/* 内存管理表 */
static MT_TYPE mem1mapbase[MEM1_ALLOC_TABLE_SIZE];                                                          /* AXISRAM1~2内存池MAP */
static MT_TYPE mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section(".EXTRAM")));                      /* XSPI1 HyperRAM内存池MAP */
