...
extern void LL_ATON_RT_DebugDumpState(const NN_Instance_TypeDef *nn_instance);

#ifndef AI_STAI_RUN_DEBUG
#define AI_STAI_RUN_DEBUG 1
#endif
#if AI_STAI_RUN_DEBUG
#define AI_STAI_RUN_PRINTF(...) do { if (AI_STAI_RUN_DEBUG) printf(__VA_ARGS__); } while (0)
#else
#define AI_STAI_RUN_PRINTF(...) do { } while (0)
#endif
...
__attribute__((unused)) static const char *__ll_aton_stai_rt_ret_name(LL_ATON_RT_RetValues_t ret)
{
  switch (ret)
  {
  case LL_ATON_RT_NO_WFE:
...
STAI_INTERNAL_ENTRY
static stai_return_code __ll_aton_stai_run_synchonously(NN_Instance_TypeDef *nn_instance)
{
  STAI_ASSERT(nn_instance != NULL);
  ...
  const uint32_t timeout_count = 10000UL;

  do
  {
    const LL_ATON_RT_EpochBlockItem_t *eb_before;
    const LL_ATON_RT_EpochBlockItem_t *eb_after;
    uint32_t flags_after;

    loop_count++;

    eb_before = nn_instance->exec_state.current_epoch_block;
    ...
    eb_after = nn_instance->exec_state.current_epoch_block;
    flags_after = (eb_after != NULL) ? eb_after->flags : 0UL;

    if ((loop_count <= 20U) || ((loop_count % 100U) == 0U) || (ll_aton_rt_ret == LL_ATON_RT_DONE))
    {
      AI_STAI_RUN_PRINTF("[STAI_RUN] loop=%lu ret=%d status=%d eb=%p flags=0x%08lx\r\n",
                         (unsigned long)loop_count,
                         (int)ll_aton_rt_ret,
                         (int)stai_nn_ctx->exec_status,
                         eb_after,
    ...
    if (loop_count > timeout_count)
    {
      eb_after = nn_instance->exec_state.current_epoch_block;
      flags_after = (eb_after != NULL) ? eb_after->flags : 0UL;
      AI_STAI_RUN_PRINTF("[STAI_RUN] TIMEOUT loop=%lu ret=%d status=%d eb=%p flags=0x%08lx start=%p end=%p prev=%p\r\n",
                         (unsigned long)loop_count,
                         (int)ll_aton_rt_ret,
                         (int)stai_nn_ctx->exec_status,
                         eb_after,
                         (unsigned long)flags_after,
                         (eb_after != NULL) ? (void *)eb_after->start_epoch_block : NULL,
                         (eb_after != NULL) ? (void *)eb_after->end_epoch_block : NULL,
                         eb_before);
      LL_ATON_RT_DebugDumpState(nn_instance);
      stai_nn_ctx->first_error = STAI_ERROR_GENERIC;
      stai_nn_ctx->exec_status = STAI_ERROR_GENERIC;
  ...
  case STAI_MODE_SYNC:
    {
      ...
      run_ret = __ll_aton_stai_run_synchonously(&nn_context->network_instance); // run current network
      if (run_ret != STAI_SUCCESS)
      {
        return run_ret;
      }
    }
