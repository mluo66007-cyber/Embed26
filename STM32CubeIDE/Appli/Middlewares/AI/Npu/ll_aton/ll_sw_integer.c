...
#ifndef AI_SW_INTEGER_DEBUG
#define AI_SW_INTEGER_DEBUG 0
#endif
#if AI_SW_INTEGER_DEBUG
#define AI_SW_INTEGER_PRINTF(...) do { if (AI_SW_INTEGER_DEBUG) printf(__VA_ARGS__); } while (0)
#else
#define AI_SW_INTEGER_PRINTF(...) do { } while (0)
#endif
#include "ll_sw.h"
...
{
  const uint32_t n = sw_info->general.output.dim.num_elem;
  const uint32_t scale_count = sw_info->is.dim.num_elem ? sw_info->is.dim.num_elem : 1U;
  const float *scale = (const float *)sw_info->is.mem.start_offset;
  float *out = (float *)sw_info->general.output.mem.start_offset;

  AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] enter in=%p scale=%p zp=%p out=%p n=%lu scales=%lu in_signed=%d zp_signed=%d\r\n",
         sw_info->general.input.mem.start_offset,
         sw_info->is.mem.start_offset,
         sw_info->izp.mem.start_offset,
         sw_info->general.output.mem.start_offset,
         (unsigned long)n,
         (unsigned long)scale_count,
         sw_info->general.input.format.is_signed,
         sw_info->izp.format.is_signed);

  AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] probe scale read\r\n");
  volatile float scale0_probe = scale[0];
  AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] probe scale read done raw=%08lx\r\n", (unsigned long)(*((const uint32_t *)&scale0_probe)));

  AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] probe zp read\r\n");
  volatile int32_t zp0_probe = sw_info->izp.format.is_signed ? (int32_t)(*((const int8_t *)sw_info->izp.mem.start_offset))
                                                             : (int32_t)(*((const uint8_t *)sw_info->izp.mem.start_offset));
  AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] probe zp read done val=%ld\r\n", (long)zp0_probe);

  AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] probe input read\r\n");
  volatile int32_t in0_probe = sw_info->general.input.format.is_signed
                                   ? (int32_t)(*((const int8_t *)sw_info->general.input.mem.start_offset))
                                   : (int32_t)(*((const uint8_t *)sw_info->general.input.mem.start_offset));
  AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] probe input read done val=%ld\r\n", (long)in0_probe);

  AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] probe output write\r\n");
  out[0] = ((float)(in0_probe - zp0_probe)) * scale0_probe;
  AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] probe output write done out0_raw=%08lx\r\n", (unsigned long)(*((const uint32_t *)&out[0])));

  if (sw_info->general.input.format.is_signed)
  {
    const int8_t *in = (const int8_t *)sw_info->general.input.mem.start_offset;

    if (sw_info->izp.format.is_signed)
    {
      const int8_t *zp = (const int8_t *)sw_info->izp.mem.start_offset;
      for (uint32_t i = 0; i < n; i++)
      {
        if ((i & 0x7FFFU) == 0U)
        {
          AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] loop i=%lu\r\n", (unsigned long)i);
        }
        const uint32_t q = (scale_count == 1U) ? 0U : (i % scale_count);
        out[i] = ((float)((int32_t)in[i] - (int32_t)zp[q])) * scale[q];
      }
    }
    else
    {
      const uint8_t *zp = (const uint8_t *)sw_info->izp.mem.start_offset;
      for (uint32_t i = 0; i < n; i++)
      {
        if ((i & 0x7FFFU) == 0U)
        {
  ...
  {
    const uint8_t *in = (const uint8_t *)sw_info->general.input.mem.start_offset;
  ...
  AI_SW_INTEGER_PRINTF("[LL_SW_DEQ] exit out0=%f\r\n", (double)out[0]);
}
