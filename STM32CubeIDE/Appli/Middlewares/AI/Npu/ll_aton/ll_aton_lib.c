...
#ifndef AI_LIB_DMA_DEBUG
#define AI_LIB_DMA_DEBUG 1
#endif

#if AI_LIB_DMA_DEBUG
#define AI_LIB_PRINTF(...) printf(__VA_ARGS__)
#else
#define AI_LIB_PRINTF(...)                                                                                              \
  do                                                                                                                    \
  {                                                                                                                     \
  } while (0)
#endif
...
static void __ll_lib_debug_dump_dma(const char *tag, const LL_Streng_TensorInitTypeDef *dma)
{
#if AI_LIB_DMA_DEBUG
  AI_LIB_PRINTF("[LIB_DMA] %s dir=%u raw=%u cont=%u base=%p start=%p end=%p limit=%p len=%lu off=%lu/%lu/%lu "
                "fw=%lu fh=%lu bd=%lu bo=%lu fo=%lu lo=%lu flc=%lu ftc=%lu nbits=%u/%u cache=%u cid=%u\r\n",
                tag, (unsigned int)dma->dir, (unsigned int)dma->raw, (unsigned int)dma->continuous,
                dma->addr_base.p, LL_Streng_addr_start(dma), LL_Streng_addr_end(dma), LL_Streng_addr_limit(dma),
                (unsigned long)LL_Streng_len(dma), (unsigned long)dma->offset_start, (unsigned long)dma->offset_end,
                (unsigned long)dma->offset_limit, (unsigned long)dma->fwidth, (unsigned long)dma->fheight,
                (unsigned long)dma->batch_depth, (unsigned long)dma->batch_offset, (unsigned long)dma->frame_offset,
                (unsigned long)dma->line_offset, (unsigned long)dma->frame_loop_cnt,
                (unsigned long)dma->frame_tot_cnt, (unsigned int)dma->nbits_in, (unsigned int)dma->nbits_out,
                (unsigned int)dma->cacheable, (unsigned int)dma->bus_cid);
#else
  LL_ATON_LIB_UNUSED(tag);
  LL_ATON_LIB_UNUSED(dma);
#endif
}

static void __ll_lib_debug_dump_params(const char *tag, const __ll_lib_params_t *params)
{
#if AI_LIB_DMA_DEBUG
  AI_LIB_PRINTF("[LIB_DMA] %s idx=%lu/%lu size=%ld wait=0x%08lx dst_o_src=%p not_cont=%lu off_limit=%lu "
                "dma_in_id=%d dma_out_id=%d\r\n",
                tag, (unsigned long)params->g_idx, (unsigned long)params->g_num_tensors,
                (long)params->g_size, (unsigned long)params->g_wait_mask, params->g_dst_o_src,
                (unsigned long)params->g_not_continuous, (unsigned long)params->g_offset_limit,
                __ll_lib_dma_unit_id[1], __ll_lib_dma_unit_id[0]);
  __ll_lib_debug_dump_dma("dma_in", &params->g_dma_in);
  __ll_lib_debug_dump_dma("dma_out", &params->g_dma_out);
#else
  LL_ATON_LIB_UNUSED(tag);
  LL_ATON_LIB_UNUSED(params);
#endif
}
...
{
  AI_LIB_PRINTF("[LIB_DMA] strswitch_set_dmas enter dma_in=%d dma_out=%d eb_array=%p\r\n", dma_in, dma_out,
                epoch_block_array);
  ...
  AI_LIB_PRINTF("[LIB_DMA] strswitch_set_dmas wait_mask=0x%08lx epoch_wait=0x%08lx\r\n",
                (unsigned long)params->g_wait_mask, (unsigned long)epoch_block_array->wait_mask);
...
{
  AI_LIB_PRINTF("[LIB_DMA] start_transfer enter wait=0x%08lx dma_in_id=%d dma_out_id=%d\r\n",
                (unsigned long)params->g_wait_mask, __ll_lib_dma_unit_id[1], __ll_lib_dma_unit_id[0]);
  __ll_lib_debug_dump_params("start_transfer params", params);
  AI_LIB_PRINTF("[LIB_DMA] before LL_Streng_TensorInit in id=%d\r\n", __ll_lib_dma_unit_id[1]);
  int ret_in = LL_Streng_TensorInit(__ll_lib_dma_unit_id[1], &params->g_dma_in, 1);
  AI_LIB_PRINTF("[LIB_DMA] after LL_Streng_TensorInit in ret=%d\r\n", ret_in);
  AI_LIB_PRINTF("[LIB_DMA] before LL_Streng_TensorInit out id=%d\r\n", __ll_lib_dma_unit_id[0]);
  int ret_out = LL_Streng_TensorInit(__ll_lib_dma_unit_id[0], &params->g_dma_out, 1);
  AI_LIB_PRINTF("[LIB_DMA] after LL_Streng_TensorInit out ret=%d\r\n", ret_out);
  AI_LIB_PRINTF("[LIB_DMA] before LL_Switch_Init\r\n");
  LL_Switch_Init(__ll_switch_init, 1);
  AI_LIB_PRINTF("[LIB_DMA] after LL_Switch_Init\r\n");
  AI_LIB_PRINTF("[LIB_DMA] before LL_ATON_EnableUnits_Init\r\n");
  LL_ATON_EnableUnits_Init(__ll_lib_dma_units, 2);
  AI_LIB_PRINTF("[LIB_DMA] after LL_ATON_EnableUnits_Init\r\n");
}

static inline void __ll_lib_stop_transfer(void)
{
  AI_LIB_PRINTF("[LIB_DMA] stop_transfer enter dma_in_id=%d dma_out_id=%d\r\n", __ll_lib_dma_unit_id[1],
                __ll_lib_dma_unit_id[0]);
  LL_Switch_Deinit(__ll_switch_init, 1);
  AI_LIB_PRINTF("[LIB_DMA] after LL_Switch_Deinit\r\n");
  LL_ATON_DisableUnits_Init(__ll_lib_dma_units, 2);
  AI_LIB_PRINTF("[LIB_DMA] after LL_ATON_DisableUnits_Init\r\n");
  __LL_ATON_RT_SetWaitMask(0);
  AI_LIB_PRINTF("[LIB_DMA] stop_transfer exit\r\n");
}
...
static inline uint32_t __ll_lib_set_wait_mask(LL_ATON_RT_EpochBlockItem_t *eb, uint32_t wait_mask)
{
  ...
  AI_LIB_PRINTF("[LIB_DMA] set_wait_mask eb=%p old=0x%08lx new=0x%08lx\r\n", eb, (unsigned long)previous_value,
                (unsigned long)wait_mask);
  eb->wait_mask = wait_mask;

  __LL_ATON_RT_SetWaitMask(eb->wait_mask);
  AI_LIB_PRINTF("[LIB_DMA] set_wait_mask done eb=%p now=0x%08lx\r\n", eb, (unsigned long)eb->wait_mask);
...
static void __ll_lib_outputs_memcpy_start(const LL_ATON_RT_EpochBlockItem_t *epoch_block, uint8_t *_dst)
{
  ...
  size_t n;
  AI_LIB_PRINTF("[LIB_DMA] outputs_memcpy_start enter eb=%p dst=%p src=%p\r\n", epoch_block, _dst, _src);
  __ll_lib_debug_dump_params("outputs_memcpy_start params before", params);
  ...
  AI_LIB_PRINTF("[LIB_DMA] outputs_memcpy_start computed n=%lu g_size=%ld not_cont=%lu\r\n", (unsigned long)n,
                (long)params->g_size, (unsigned long)params->g_not_continuous);
  if (params->g_not_continuous == 0)
  {
    AI_LIB_PRINTF("[LIB_DMA] outputs_memcpy_start before prolog dst=%p src=%p n=%lu\r\n", _dst, _src,
                  (unsigned long)n);
    n = __ll_lib_memcpy_prolog((void **)&_dst, (void **)&_src, n);
    AI_LIB_PRINTF("[LIB_DMA] outputs_memcpy_start after prolog dst=%p src=%p n=%lu\r\n", _dst, _src,
                  (unsigned long)n);
  }
  ...
  if (n > 0)
  {
    ...
    __ll_lib_debug_dump_params("outputs_memcpy_start params before wait", params);
    __ll_lib_set_wait_mask((LL_ATON_RT_EpochBlockItem_t *)epoch_block, params->g_wait_mask);
    AI_LIB_PRINTF("[LIB_DMA] outputs_memcpy_start before start_transfer eb_wait=0x%08lx\r\n",
                  (unsigned long)epoch_block->wait_mask);
    __ll_lib_start_transfer(params);
    AI_LIB_PRINTF("[LIB_DMA] outputs_memcpy_start after start_transfer\r\n");
  }
  else
  {
    /* do not start any transfer and wait, just proceed to end function */
    AI_LIB_PRINTF("[LIB_DMA] outputs_memcpy_start n=0 set wait 0\r\n");
    __ll_lib_set_wait_mask((LL_ATON_RT_EpochBlockItem_t *)epoch_block, 0);
  }
  AI_LIB_PRINTF("[LIB_DMA] outputs_memcpy_start exit eb_wait=0x%08lx\r\n", (unsigned long)epoch_block->wait_mask);
}
                                                                    ...
                                                                    const NN_Instance_TypeDef *nn_instance)
{
  ...
  __ll_lib_params_t *params = __ll_lib_get_params();
  AI_LIB_PRINTF("[LIB_DMA] OCSB start enter eb=%p flags=0x%08lx wait=0x%08lx\r\n", epoch_block,
                (unsigned long)epoch_block->flags, (unsigned long)epoch_block->wait_mask);
  __ll_lib_debug_dump_params("OCSB start params enter", params);
  LL_ATON_ASSERT(params->g_idx < params->g_num_tensors); // must be checked before
  params->g_not_continuous =
      1; // disables code in __ll_lib_outputs_memcpy_start that assumes a flat copy operation e.g. prolog test

  LL_Buffer_InfoTypeDef *out = ((LL_Buffer_InfoTypeDef *)params->g_tensors) + params->g_idx;
  uint8_t *dst = (uint8_t *)LL_Buffer_addr_start(out);
  AI_LIB_PRINTF("[LIB_DMA] OCSB start tensor idx=%lu out=%p addr_base=%p off=%lu/%lu/%lu len=%lu ndims=%lu "
                "shape=%lu,%lu,%lu,%lu nbits=%lu batch=%lu dst=%p\r\n",
                (unsigned long)params->g_idx, out, out->addr_base.p, (unsigned long)out->offset_start,
                (unsigned long)out->offset_end, (unsigned long)out->offset_limit, (unsigned long)LL_Buffer_len(out),
                (unsigned long)out->ndims, (unsigned long)out->shape[0], (unsigned long)out->shape[1],
                (unsigned long)out->shape[2], (unsigned long)out->shape[3], (unsigned long)out->nbits,
                (unsigned long)out->batch, dst);
  __ll_lib_outputs_memcpy_start(epoch_block, dst);
  AI_LIB_PRINTF("[LIB_DMA] OCSB start exit eb_wait=0x%08lx\r\n", (unsigned long)epoch_block->wait_mask);
}
                                                                  ...
                                                                  const NN_Instance_TypeDef *nn_instance)
{
  ...
  __ll_lib_params_t *params = __ll_lib_get_params();
  AI_LIB_PRINTF("[LIB_DMA] OCSB end enter eb=%p flags=0x%08lx wait=0x%08lx\r\n", epoch_block,
                (unsigned long)epoch_block->flags, (unsigned long)epoch_block->wait_mask);
  __ll_lib_debug_dump_params("OCSB end params enter", params);

  if (__ll_lib_set_wait_mask((LL_ATON_RT_EpochBlockItem_t *)epoch_block, 0))
  {
    AI_LIB_PRINTF("[LIB_DMA] OCSB end stopping transfer\r\n");
    __ll_lib_stop_transfer();
  }
  ...
  int out_rank_old = out->ndims;
  int out_nchannels_old = out->shape[(out_rank_old - 4) + 1 /* ONNX_CHANNEL_OFFSET */];
  AI_LIB_PRINTF("[LIB_DMA] OCSB end completed idx=%lu old_out=%p old_ch=%d len=%lu\r\n",
                (unsigned long)params->g_idx, out, out_nchannels_old, (unsigned long)LL_Buffer_len(out));

  params->g_idx++;
  AI_LIB_PRINTF("[LIB_DMA] OCSB end next idx=%lu num=%lu\r\n", (unsigned long)params->g_idx,
                (unsigned long)params->g_num_tensors);
  ...
  if (params->g_idx < params->g_num_tensors)
  {
    ...
    AI_LIB_PRINTF("[LIB_DMA] OCSB end next tensor out=%p addr_base=%p off=%lu/%lu/%lu len=%lu ndims=%lu "
                  "shape=%lu,%lu,%lu,%lu nbits=%lu batch=%lu\r\n",
                  out, out->addr_base.p, (unsigned long)out->offset_start, (unsigned long)out->offset_end,
                  (unsigned long)out->offset_limit, (unsigned long)LL_Buffer_len(out), (unsigned long)out->ndims,
                  (unsigned long)out->shape[0], (unsigned long)out->shape[1], (unsigned long)out->shape[2],
                  (unsigned long)out->shape[3], (unsigned long)out->nbits, (unsigned long)out_batch);
    ...
    unsigned int frame_size = out_fwidth * out_fheight * batch_depth;
    AI_LIB_PRINTF("[LIB_DMA] OCSB end next geom nbytes=%lu fw=%lu fh=%lu ch=%lu batch=%lu bd=%lu frame_size=%lu\r\n",
                  (unsigned long)nbytes, (unsigned long)out_fwidth, (unsigned long)out_fheight,
                  (unsigned long)out_nchannels, (unsigned long)out_batch, (unsigned long)batch_depth,
    ...
    if (frame_size != 1)
    {
      ...
      __ll_lib_debug_dump_params("OCSB end params before loopback", params);
    ...
    /* loop back one epoch block */
    AI_LIB_PRINTF("[LIB_DMA] OCSB end loopback DecCurrEpochBlock(1)\r\n");
    LL_ATON_RT_DecCurrEpochBlock(1);
    AI_LIB_PRINTF("[LIB_DMA] OCSB end loopback requested\r\n");
  }
  else
  {
    /* Release ATON lock */
    AI_LIB_PRINTF("[LIB_DMA] OCSB end release owner\r\n");
    __ll_clear_aton_owner(nn_instance, false);

    /* proceed to next epoch block */
  }
  AI_LIB_PRINTF("[LIB_DMA] OCSB end exit\r\n");
}
