...
#ifndef AI_SW_FLOAT_DEBUG
#define AI_SW_FLOAT_DEBUG 0
#endif
#if AI_SW_FLOAT_DEBUG
#define AI_SW_FLOAT_PRINTF(...) do { if (AI_SW_FLOAT_DEBUG) printf(__VA_ARGS__); } while (0)
#else
#define AI_SW_FLOAT_PRINTF(...) do { } while (0)
#endif
#include "ll_sw.h"
...
{
  AI_SW_FLOAT_PRINTF("[LL_SW_ARITH] ENTER_RAW sw_info=%p\r\n", sw_info);
  float *input = (float *)sw_info->general.input.mem.start_offset;
  float *operand = (float *)sw_info->operand.mem.start_offset;
  float *operand1 = (float *)sw_info->operand1.mem.start_offset;
  float *operand2 = (float *)sw_info->operand2.mem.start_offset;
  float *output = (float *)sw_info->general.output.mem.start_offset;
  const uint32_t n = sw_info->general.output.dim.num_elem;
  const uint32_t in_n = sw_info->general.input.dim.num_elem;
  const uint32_t op_n = sw_info->operand.dim.num_elem;
  const uint32_t op1_n = sw_info->operand1.dim.num_elem;
  const uint32_t op2_n = sw_info->operand2.dim.num_elem;

  AI_SW_FLOAT_PRINTF("[LL_SW_ARITH] enter type=%d in=%p op=%p op1=%p op2=%p out=%p n=%lu in_n=%lu op_n=%lu inputs=%u\r\n",
         (int)sw_info->general.type,
         input,
         operand,
         operand1,
         operand2,
         ...
         (unsigned long)in_n,
         (unsigned long)op_n,
         sw_info->num_of_inputs);

  if ((input != NULL) && (operand != NULL) && (output != NULL) && (n > 0U))
  {
    AI_SW_FLOAT_PRINTF("[LL_SW_ARITH] probe read/write\r\n");
    {
      volatile float probe_in = input[0];
      volatile float probe_op = operand[0];
      output[0] = probe_in;
      (void)probe_op;
      AI_SW_FLOAT_PRINTF("[LL_SW_ARITH] probe done in=%f op=%f out=%f\r\n", (double)probe_in, (double)probe_op, (double)output[0]);
    }

    for (uint32_t i = 0; i < n; i++)
    {
      const float a = input[(in_n == 1U) ? 0U : i];
      const float b = operand[(op_n == 1U) ? 0U : i];
      float y = 0.0f;

      if ((i & 0x7FFFU) == 0U)
      {
        AI_SW_FLOAT_PRINTF("[LL_SW_ARITH] loop i=%lu\r\n", (unsigned long)i);
      }
      ...
      case LL_SW_ARITHSUM:
        y = a + b;
        if ((sw_info->num_of_inputs >= 3U) && (operand1 != NULL))
        {
          y += operand1[(op1_n == 1U) ? 0U : i];
        }
        if ((sw_info->num_of_inputs >= 4U) && (operand2 != NULL))
        {
          y += operand2[(op2_n == 1U) ? 0U : i];
        }
        break;
      case LL_SW_ARITHSUB:
        y = a - b;
      ...
      case LL_SW_MAX:
        ...
        break;
      default:
        AI_SW_FLOAT_PRINTF("[LL_SW_ARITH] unsupported direct type=%d\r\n", (int)sw_info->general.type);
        return;
      }
    ...
    AI_SW_FLOAT_PRINTF("[LL_SW_ARITH] exit out0=%f\r\n", (double)output[0]);
    return;
  }

  AI_SW_FLOAT_PRINTF("[LL_SW_ARITH] invalid args in=%p op=%p out=%p n=%lu\r\n",
         input, operand, output, (unsigned long)n);
  return;
