...
{
    if ((len == 0) || (x0 > rgblcddev.width) || (y0 > rgblcddev.height))return;
    rgblcd_fill(x0, y0, x0 + len - 1, y0, color);
}
...
{
    uint16_t i, j;
    if (rgblcddev.pwidth != 0 && rgblcddev.dir == 0)
    {
        for (i =0; i < height; i++)
        {
            for (j = 0; j < width; j++)
            {
                *(uint16_t *)((uint32_t)g_ltdc_lcd_framebuf + 2 * (rgblcddev.pwidth * (rgblcddev.pheight - x - j - 1) + y + i)) = color[i * width + j];
            }
        }
    }
    else
    {
        rgblcd_color_fill(x, y, x + width - 1, y + height - 1, color);     /* 填充 */
    }
}
...
{
    pic_phy.read_point = rgblcd_read_point;    /* 读点函数实现,仅BMP需要 */
    pic_phy.draw_point = rgblcd_draw_point;    /* 画点函数实现 */
    pic_phy.fill = rgblcd_fill;                /* 填充函数实现,仅GIF需要 */
    pic_phy.draw_hline = rgblcd_draw_hline;    /* 画线函数实现,仅GIF需要 */
    pic_phy.fillcolor = piclib_fill_color;  /* 颜色填充函数实现,仅TJPGD需要 */

    picinfo.lcdwidth = rgblcddev.width;        /* 得到LCD的宽度像素 */
    picinfo.lcdheight = rgblcddev.height;      /* 得到LCD的高度像素 */
...
uint8_t piclib_ai_load_picfile(char *filename, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t fast)
{
    ...
    {
        picinfo.S_Height = rgblcddev.height;
        picinfo.S_Width = rgblcddev.width;
        return FALSE;
    }
    ...
    switch (temp)
    {
        ...
        case T_JPEG:
            if (fast)   /* 可能需要硬件解码 */
            {
                res = jpg_get_size((const uint8_t *)filename, &picinfo.ImgWidth, &picinfo.ImgHeight);

                if (res == 0)
                {
#ifdef HJPEG
                    if (picinfo.ImgWidth <= rgblcddev.width && picinfo.ImgHeight <= rgblcddev.height &&       /* 满足分辨率小于等于屏幕分辨率 */
