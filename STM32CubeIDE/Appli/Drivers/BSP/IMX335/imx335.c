...
static void imx335_dbg(uint16_t y, const char *msg)
{
    rgblcd_show_string(30, y, 460, 16, 16, (char *)msg, BLUE);
    HAL_Delay(200);
}
...
uint8_t imx335_init(void)
{
    ...
    imx335_dbg(255, "I0: imx init enter");

    imx335_dbg(275, "I1: before dcmipp");
    if (imx335_dcmipp_init() != 0)
    {
        imx335_dbg(295, "ERR: dcmipp init");
        return 1;
    }
    imx335_dbg(295, "I2: after dcmipp");
    ...
    imx335_dbg(315, "I3: before BusIO");
    if (IMX335_RegisterBusIO(&imx335_object, &imx335_io_struct) != IMX335_OK)
    {
        imx335_dbg(335, "ERR: BusIO");
        return 1;
    }
    imx335_dbg(335, "I4: after BusIO");

    imx335_dbg(355, "I5: before ReadID");
    if (IMX335_ReadID(&imx335_object, &id) != IMX335_OK)
    {
        imx335_dbg(375, "ERR: ReadID");
        return 1;
    }
    imx335_dbg(375, "I6: after ReadID");

    if (id != (uint32_t)IMX335_ID)
    {
        imx335_dbg(395, "ERR: wrong ID");
        return 1;
    }

    imx335_dbg(395, "I7: before sensor init");
    if (IMX335_Init(&imx335_object, IMX335_R2592_1944, IMX335_RAW_RGGB10) != IMX335_OK)
    {
        imx335_dbg(415, "ERR: sensor init");
        return 1;
    }
    imx335_dbg(415, "I8: after sensor init");

    imx335_dbg(435, "I9: before frequency");
    if (IMX335_SetFrequency(&imx335_object, IMX335_INCK_37MHZ) != IMX335_OK)
    {
        imx335_dbg(455, "ERR: set frequency");
        return 1;
    }
    imx335_dbg(455, "I10: after frequency");
    ...
    imx335_dbg(475, "I11: before ISP_Init");
    ISP_Init(&imx335_hisp, &hdcmipp, 0, &isp_appli_helpers_struct, &isp_stat_area_struct, ISP_IQParamCacheInit[0]);
    imx335_dbg(495, "I12: after ISP_Init");

    imx335_dbg(515, "I13: before ISP_Start");
    if (ISP_Start(&imx335_hisp) != ISP_OK)
    {
        imx335_dbg(535, "ERR: ISP_Start");
        return 1;
    }
    imx335_dbg(535, "I14: after ISP_Start");
...
static uint8_t imx335_dcmipp_init(void)
{
    ...
    imx335_dbg(275, "D1: HAL_DCMIPP_Init");
    hdcmipp.Instance = DCMIPP;
    if (HAL_DCMIPP_Init(&hdcmipp) != HAL_OK)
    {
        imx335_dbg(295, "ERR: DCMIPP_Init");
        return 1;
    }

    imx335_dbg(295, "D2: CSI config");
    ...
    imx335_dbg(300, "D2A: before CSI Set");

    HAL_StatusTypeDef csi_ret;
    csi_ret = HAL_DCMIPP_CSI_SetConfig(&hdcmipp, &dcmipp_csi_conf_struct);

    imx335_dbg(320, "D2B: after CSI Set");

    if (csi_ret != HAL_OK)
    {
        imx335_dbg(340, "ERR: CSI config");
        return 1;
    }
    imx335_dbg(315, "D3: VC config");
    if (HAL_DCMIPP_CSI_SetVCConfig(&hdcmipp, DCMIPP_VIRTUAL_CHANNEL0, DCMIPP_CSI_DT_BPP10) != HAL_OK)
    {
        imx335_dbg(335, "ERR: VC config");
        return 1;
    }

    imx335_dbg(335, "D4: pipe CSI config");
    ...
    {
        imx335_dbg(355, "ERR: pipe CSI");
        return 1;
    }

    imx335_dbg(355, "D5: pipe config");
    ...
    {
        imx335_dbg(375, "ERR: pipe config");
        return 1;
    }

    imx335_dbg(375, "D6: downsize config");
    ...
    {
        imx335_dbg(395, "ERR: downsize");
        return 1;
    }

    imx335_dbg(395, "D7: enable downsize");
    if (HAL_DCMIPP_PIPE_EnableDownsize(&hdcmipp, DCMIPP_PIPE1) != HAL_OK)
    {
        imx335_dbg(415, "ERR: enable downsize");
        return 1;
    }

    imx335_dbg(415, "D8: dcmipp done");
    return 0;
}
