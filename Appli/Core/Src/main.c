...
static void show_sensor_page(uint16_t ir, uint16_t ps, uint16_t als, uint32_t moisture_raw, uint8_t shade_state)
{
  char buf[48];

  lcd_clear_simple(BLACK);

  rgblcd_show_string(30, 40, 360, 16, 16, "SENSOR DATA", RED);

  sprintf(buf, "IR    : %5u", ir);
  rgblcd_show_string(30, 85, 360, 16, 16, buf, BLUE);

  sprintf(buf, "PS    : %5u", ps);
  rgblcd_show_string(30, 115, 360, 16, 16, buf, BLUE);

  sprintf(buf, "ALS   : %5u", als);
  rgblcd_show_string(30, 145, 360, 16, 16, buf, BLUE);

  sprintf(buf, "MOIST : %5lu", (unsigned long)moisture_raw);
  rgblcd_show_string(30, 175, 360, 16, 16, buf, BLUE);

  sprintf(buf, "SHADE : %s", shade_state_text(shade_state));
  rgblcd_show_string(30, 205, 360, 16, 16, buf, GREEN);

  rgblcd_show_string(30, 250, 360, 16, 16, "KEY1: Back to Camera", GREEN);
}
    ...
    while (1)
    {
      ...
      if (key == KEY1_PRES)
      {
        if (display_mode == 0)
        {
          ...
          show_sensor_page(ir, ps, als, moisture_raw, shade_state);
        }
        else
        {
          display_mode = 0;
      ...
      if (++ap_tick >= 50)
      {
        ...
        printf("IR:%u  PS:%u  ALS:%u  Moisture:%lu  Shade:%s\r\n",
               ir, ps, als, (unsigned long)moisture_raw, shade_state_text(shade_state));
